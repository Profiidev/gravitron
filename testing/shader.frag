...
float geometry(vec3 light_direction, vec3 cam_direction, float roughness2) {
  float n_dot_l = abs(dot(normal, light_direction));
  float n_dot_c = abs(dot(normal, cam_direction));
  return 0.5 / max(0.01, mix(2 * n_dot_l * n_dot_c, n_dot_l + n_dot_c, roughness2));
}

float distribution(vec3 half_vector, float roughness2) {
...
vec3 compute_light(vec3 light_color, vec3 light_direction, vec3 color, vec3 cam_direction) {
  ...
  vec3 relevant_reflection = reflected_color * f * geometry(light_direction, cam_direction, roughness2) * distribution(half_vec, roughness2);

  return refracted_not_absorbed_color * color / PI + relevant_reflection;
}

void main() {
  vec4 color = texture(tex, uv) + color_in;
